...
import ru.aafanasiev.util.copier.converter.clong.CopyConverterLong2BigDecimal;

/**
 * @author aafanasyev
 */
@Test(groups = {"UTIL"}, ignoreMissingDependencies = true)
public class CopyConverterLong2BigDecimalTest extends CopyConverterLongBase {
    @BeforeMethod
    public void intitialize() {
        convertor = new CopyConverterLong2BigDecimal();
        entry = new CopyEntry();
    }
    ...
    public void testConvert() throws Exception {
        BigDecimal result = (BigDecimal) convertor.convert(entry, 0L);
        assertEquals(result, BigDecimal.valueOf(0));

        result = (BigDecimal) convertor.convert(entry, Long.valueOf(1L));
        assertEquals(result, BigDecimal.valueOf(1));

        result = (BigDecimal) convertor.convert(entry, Long.MAX_VALUE);
        assertEquals(result, BigDecimal.valueOf(Long.MAX_VALUE));

        result = (BigDecimal) convertor.convert(entry, Long.MIN_VALUE);
