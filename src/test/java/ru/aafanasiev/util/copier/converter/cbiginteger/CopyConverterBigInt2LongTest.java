...
public class CopyConverterBigInt2LongTest extends CopyConverterBigIntBase {
    @BeforeMethod
    public void intitialize() {
        convertor = new CopyConverterBigInt2Long();
        entry = new CopyEntry();
    }
    ...
    public void testConvert() throws Exception {
        Long result = (Long) convertor.convert(entry, BigInteger.valueOf(0));
        assertNotNull(result);
        assertEquals(result, Long.valueOf(0));

        result = (Long) convertor.convert(entry, BigInteger.valueOf(1));
        assertEquals(result, Long.valueOf(1));

        result = (Long) convertor.convert(entry, BigInteger.valueOf(-1));
        assertEquals(result, Long.valueOf(-1));

        result = (Long) convertor.convert(entry, BigInteger.valueOf(Long.MAX_VALUE));
        assertEquals(result, Long.valueOf(Long.MAX_VALUE));

        result = (Long) convertor.convert(entry, BigInteger.valueOf(Long.MIN_VALUE));
