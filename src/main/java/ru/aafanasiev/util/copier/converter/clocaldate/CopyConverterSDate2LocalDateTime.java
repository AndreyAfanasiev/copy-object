 ...
 */
@Converter
public class CopyConverterSDate2LocalDateTime implements CopyConverter {
    ...
    public Object convert(CopyEntry entry, Object value) {
        if(null != value && Date.class.equals(value.getClass())) {
            return Instant.ofEpochMilli(((Date) value).getTime()).atZone(ZoneId.systemDefault()).toLocalDateTime();
        }
        return null;
    }
    ...
    public ConverterKey[] listConvertedPairs() {
        return new ConverterKey[] {new ConverterKey(Date.class, LocalDateTime.class)};
    }
